...
public class testaccount {
    ...
    private static void createAccount(Scanner scanner, int accountType) {
        ...
        //inputs the name variable into the methods to normalize and validate it
        name = normalize(name);
        try{
        	validatesname(name);
        } //tries to catch inputs of the user for name with specified pattern so code does not end abruptly
        catch (IllegalStateException e) {
        	System.out.println("Invalid Input for name " + e.getMessage());
        	return;
        }
	...
	//method to validate the name variable
	private static void validatesname(String name)
	{
		...
		if (matcher.find())
		{
			//if a invalid pattern is found this message will be printed
		System.out.println("Invalid pattern, exception");
			throw new IllegalStateException();
		}
		else {
			//if input is valid this message will print
			System.out.println("Input is successful");
		}
	}
	
	private static String outputencoding(String input)
	{
    ...
    private static void withdraw(Account_S2024_Group2 account, double amount) throws FormatAmountException {
        ...
        if (amount > (account.getBalance() + account.getAddOn())) {
